...
/*   Updated: 2025/11/16 18:16:28 by asharafe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	{
		if (cache[fd])
			n_address = ft_strchr(cache[fd], '\n');
		else
			n_address = NULL;
		if (n_address)
			return (ft_extract_line(&cache[fd], cache[fd], n_address));
		buffer = (char *)malloc(sizeof(char) * (BUFFER_SIZE + 1));
		if (!buffer)
			return (NULL);
		read_bytes = read(fd, buffer, BUFFER_SIZE);
		if (read_bytes <= 0)
			return (ft_handle_read_results(&cache[fd], &buffer, read_bytes));
		if (!ft_append_read(&cache[fd], &buffer, read_bytes))
			return (NULL);
	}
}
